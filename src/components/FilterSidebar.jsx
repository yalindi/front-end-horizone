...
const FilterSidebar = ({
  ...
  // CHANGED: Now storing location NAMES instead of IDs
  const [selectedLocations, setSelectedLocations] = useState(
  ...
  useEffect(() => {
    setPriceRange({ min: minPrice || '', max: maxPrice || '' });
    // CHANGED: Parse the location string from URL back to array
  ...
  // CHANGED: Simplified - now working with location names directly
  const handleLocationToggle = (locationName) => {
    ...
    console.log('Selected locations:', newLocations); // DEBUG
          ...
          {filteredLocations.map(location => (
            <div key={location._id} className="flex items-center">
              <input
                type="checkbox"
                id={`location-${location._id}`}
                checked={selectedLocations.includes(location.name)} // CHANGED: Check by name
                onChange={() => handleLocationToggle(location.name)} // CHANGED: Pass name
                className="rounded border-gray-300 text-primary focus:ring-primary"
              />
              <label
                htmlFor={`location-${location._id}`}
